...
def GM_common_build(dir):
    """GM build common directory for version of the chain
    ...
    # web3sdk
    shutil.copytree(path.get_path() + '/tpl/GM_temp_node/web3sdk', com_dir + '/web3sdk')
    # copy ca.crt to web3sdk conf dir
...
def GM_chain_build(cfg, fisco_path):
    """parser input config file, build GM install pacakge by 
    ...
    logger.debug('GM build cfg is %s, fisco is %s ', cfg, fisco_path)
    ...
    logger.debug('GM build cfg end.')


def GM_build_cfg(cc):
    """build all GM install package for one chain base on cc 
    ...
    logger.info('GM building, cc is %s', cc)
    ...
    consoler.info('\t\t build GM install package for chain %s version %s',
                  cc.get_chain().get_id(), cc.get_chain().get_version())
    ...
    if os.path.isdir(dir):
        ...
        consoler.error(' GM build install package for chain %s version %s failed, version data aleady exist, please change the version of the chain!!!.',
                       cc.get_chain().get_id(), cc.get_chain().get_version())
    ...
    try:
        ...
        # create temp node for export genesis.json file
        temp_node.GM_temp_node_build(dir, port)

        # start temp node
        if not temp_node.start_temp_node(dir, port):
            raise Exception('GM temp node start failed.')

        # build install dir for every server
        for node in cc.get_nodes():
            build.GM_build_install_dir(dir, chain, port, node, temp_node)
